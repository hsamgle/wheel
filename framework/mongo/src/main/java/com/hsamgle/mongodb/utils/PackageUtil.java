 ...
 * 
 *  @feture   :	    TODO		
 *	@file_name:	    PackageUtil.java
 * 	@packge:	    com.hsamgle.mongodb.utils
 *	@author:	    黄鹤老板 
 *  @create_time:	2018/5/21 9:39
 *	@company:		江南皮革厂
 */
public class PackageUtil {
	...
	private static Set<String> checkPackage(String scanPackages) {
        ...
        for (String pInArr : packages.toArray(new String[packages.size()])) {
            if (StringUtils.isEmpty(pInArr) || ".".equals(pInArr) || pInArr.startsWith(".")) {
                continue;
            }
            if (pInArr.endsWith(".")) {
     ...
     * @param fullClassName
     */
    private static Set<Method> findAnnotationMethods(String fullClassName, Class<? extends Annotation> anno) throws ClassNotFoundException {
